...
// Helper function to get background images based on template category
const getTemplateBackgroundImage = (category, templateName) => {
  const images = {
    "real-estate": [
      "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800&h=600&fit=crop&crop=center&auto=format&q=60", // Modern house
      "https://images.unsplash.com/photo-1570129477492-45c003edd2be?w=800&h=600&fit=crop&crop=center&auto=format&q=60", // Luxury interior
    ],
    event: [
      "https://images.unsplash.com/photo-1511578314322-379afb476865?w=800&h=600&fit=crop&crop=center&auto=format&q=40", // Party/celebration
    ],
    business: [
      "https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&h=600&fit=crop&crop=center&auto=format&q=40", // Modern office
    ],
  };

  const categoryImages = images[category] || images["business"];
  const index =
    templateName && templateName.toLowerCase().includes("luxury") ? 1 : 0;
  ...
  useEffect(() => {
    if (flyerData) {
      ...
      flyerData.template.layout.elements.forEach((element) => {
        switch (element.id) {
          case "headline":
            initialContent[element.id] =
              flyerData.content.headline || element.content;
            break;
          case "highlights":
            const highlights =
              flyerData.content.sections.find(
                (section) =>
            ...
            const locationSection =
              flyerData.content.sections.find(
                (section) =>
                  ...
                  section.title.toLowerCase().includes("choose")
              )?.content || element.content;
            initialContent[element.id] = locationSection;
            break;
          case "cta":
            initialContent[element.id] =
              flyerData.content.callToAction || element.content;
            break;
          case "details":
          case "services":
              ...
              <div className="p-8 bg-gradient-to-b from-gray-50 to-white">
                <div className="flex justify-center">
                  <div
                    ...
                    style={{
                      width: Math.min(
                        flyerData.template.layout.width * 0.7,
                        595
                      ),
                      height: Math.min(
                        flyerData.template.layout.height * 0.7,
                        770
                      ),
                      backgroundColor:
                        flyerData.template.layout.backgroundColor,
                      boxShadow:
                  ...
                  >
                    ...
                    <div className="absolute inset-0 opacity-8">
                      <div
                        ...
                        style={{
                          backgroundImage: `
                          radial-gradient(circle at 20% 50%, ${getTemplateAccentColor(
                            flyerData.template.category
                          )}15 0%, transparent 50%), 
                          radial-gradient(circle at 80% 20%, ${getTemplateAccentColor(
                            flyerData.template.category
                          )}12 0%, transparent 50%),
                          radial-gradient(circle at 40% 80%, ${getTemplateAccentColor(
                            flyerData.template.category
                          )}08 0%, transparent 50%),
                          linear-gradient(45deg, transparent 48%, ${getTemplateAccentColor(
                            flyerData.template.category
                          )}05 49%, ${getTemplateAccentColor(
                            flyerData.template.category
                          )}05 51%, transparent 52%)
                        `,
                        }}
                      ></div>
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${getTemplateAccentColor(
                          flyerData.template.category
                        )} 0%, transparent 70%)`,
                      }}
                    ></div>
                    <div
                      ...
                      style={{
                        background: `linear-gradient(315deg, ${getTemplateAccentColor(
                          flyerData.template.category
                        )} 0%, transparent 70%)`,
                      }}
                    ></div>
                        ...
                        <div
                          ...
                          style={{
                            ...
                            textShadow: isHeadline
                              ? "0 2px 8px rgba(0,0,0,0.3), 0 1px 3px rgba(255,255,255,0.8)"
                            ...
                            background:
                              selectedElement?.id === element.id
                                ...
                                : isCTA
                                ? `linear-gradient(135deg, ${getTemplateAccentColor(
                                    flyerData.template.category
                                  )}25, ${getTemplateAccentColor(
                                    flyerData.template.category
                                  )}15)`
                                : "rgba(255, 255, 255, 0.75)",
                            border: isCTA
                              ? `2px solid ${getTemplateAccentColor(
                                  flyerData.template.category
                                )}40`
                              : "none",
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <label className="block text-sm font-semibold text-gray-800">
                          Editing:{" "}
                          {selectedElement.id.charAt(0).toUpperCase() +
                            selectedElement.id.slice(1)}
                        </label>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between py-2">
                    ...
                    <span className="text-gray-900 font-mono text-sm">
                      {flyerData.template.layout.width} ×{" "}
                      {flyerData.template.layout.height}px
                    </span>
                  </div>
                </div>
              </div>
